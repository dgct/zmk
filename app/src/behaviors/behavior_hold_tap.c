...
static void decide_positional_hold(struct active_hold_tap *hold_tap) {
    // Positional conditions not active?
...
static int position_state_changed_listener(const zmk_event_t *eh) {
    ...
    // Store the position of pressed/released key for positional hold-tap purposes.
    ...
    if (!ev->state && undecided_hold_tap->position_of_first_other_key_released == -1) {
